...
struct voter_info {
  ...
  EOSLIB_SERIALIZE(
      voter_info,
      (owner)(proxy)(producers)(staked)(last_vote_weight)(proxied_vote_weight)(is_proxy)(flags1)(reserved2)(reserved3))
};
...
enum member_status : member_status_type { pending_membership = 0, active_member = 1 };
...
EOSIO_REFLECT(member_v1, base member_v0, election_participation_status, election_rank,
              representative, encryption_key);

using member_variant = std::variant<member_v0, member_v1>;

struct member {
  member_variant value;
  EDEN_FORWARD_MEMBERS(value, account, name, status, nft_template_id, election_participation_status,
                       election_rank, representative, encryption_key);
...
EOSIO_REFLECT(stats, bp, ratings_cntr, average, transparency, infrastructure, trustiness,
...
EOSIO_REFLECT(ratings, id, uniq_rating, user, bp, transparency, infrastructure, trustiness,
              development, community)
typedef eosio::multi_index<
    "ratings"_n, ratings,
...
EOSIO_REFLECT(ratings_v2, id, user, bp, transparency, infrastructure, trustiness, development,
              community)
typedef eosio::multi_index<
    "rating"_n, ratings_v2,
...
EOSIO_REFLECT(config, owner, version)
...
struct rateproducer : public eosio::contract {
   ...
   */
  void rate(name user, name bp, int8_t transparency, int8_t infrastructure, int8_t trustiness,
            int8_t community, int8_t development);
   ...
   *       or vote for a proxy with at least 21 votes
   */
  void rate_aux(name scope, name user, name bp, int8_t transparency, int8_t infrastructure,
   ...
   *
   */
  void save_bp_stats(name scope, name ram_payer, name bp_name, float transparency,
                     float infrastructure, float trustiness, float community, float development);
   ...
   */
  void calculate_bp_stats(name scope, name bp_name, float* transparency, float* infrastructure,
                          float* trustiness, float* community, float* development,
                          uint32_t* ratings_cntr, float* average);
   ...
   *
   */
  void update_bp_stats(name scope, name* ram_payer, name* bp_name, float* transparency,
  ...
  /**
   *
   *  Give a like/dislike to a bp comment
   *
   * @param user - Commenter account,
   * @param bp -  Block Producer account name
   * @param like - true/false for the comment
   *
   */
  void loglike(name user, name bp, bool like);
...
EOSIO_ACTIONS(rateproducer, "rateproducer"_n,
              action(rate, user, bp, transparency, infrastructure, trustiness, community,
                     development, ricardian_contract(rate_ricardian)),
              ...
              action(loglike, user, bp, like, ricardian_contract(loglike_ricardian)))
