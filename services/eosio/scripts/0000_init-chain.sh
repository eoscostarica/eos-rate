...
function create_eos_account () {
  ...
  $cleos create account eosrate $1 $2 $2
...
# Create eosrate priveledged account
function create_eosrate_account () {
  eosrate_OWNER_PVTKEY="5KacG2v3XYrjmxazgriHVo1updD7PKXJMWzcaQmBMMXE9Y69aW9"
  eosrate_OWNER_PUBKEY="EOS88bvtAMTwPBQyF8cxFUFXez9zCoebABS3dXngdNphqNtiszLQh"

  eosrate_ACTIVE_PVTKEY="5Hy5kAujsv4fVWa9xv784Pgy4eLgrrDf3trP49J3FvDpKRfzaNn"
  eosrate_ACTIVE_PUBKEY="EOS8G66UbcXKfQ7unJES7BrKHggQMZfHUkTMkMF8nEbsktpjsb9tr"

  echo "Importing eosio and eosrate keys"
  import_private_key $eosrate_OWNER_PVTKEY
  import_private_key $eosrate_ACTIVE_PVTKEY

  echo "Creates eosrate account with stake..."
  # $cleos system newaccount eosio --transfer eosrate $eosrate_OWNER_PUBKEY $eosrate_ACTIVE_PUBKEY --stake-net '1 EOS' --stake-cpu '1 EOS' --buy-ram '1 EOS'
  $cleos create account eosio eosrate $eosrate_OWNER_PUBKEY $eosrate_ACTIVE_PUBKEY
  $cleos push action eosio.token issue '[ "'eosrate'", "1000.0000 EOS", "initial stake" ]' -p eosio
  sleep .5
}
...
# build and deploy eosrate demo contract
function build_and_deploy_contracts () {
  ...
  $cleos push action eosrate rate '[]' -p eosrateusra@active
  $cleos push action eosrate rate '[]' -p eosrateusrb@active
}
